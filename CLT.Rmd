...
animate(g, fps = 20, nframes = 100, res=100, detail = 2)
anim_save("CLT_Binomial.gif")
```

## High Coins, High Tosses
```{r}
tosses = 300
p = 0.5
...
g = ggplot(data, aes(x = p_hat, group = coins)) +
  ...
  labs(title = expression(atop("Sampling Distribution -> Gaussian", scriptstyle("Tosses = 300"))),
       subtitle = "Coins = {closest_state}",
       x = expression(hat(p)),
       y = "Density\n") +
...
## High Coins, Slowly Tncreasing tosses
```{r}
coins = 1e5
p = 0.5
...
g = ggplot(data, aes(x = p_hat, group = tosses)) +
  stat_density(aes(color = as.numeric(tosses), fill = as.numeric(tosses)),
               adjust = 1.25, size = 2, alpha = 0.75, geom = "area", position = "identity", show.legend = FALSE) +
  labs(title = expression(atop("Binomial -> Gaussian", scriptstyle("Coins = 1e5"))),
...
animate(g, fps = 30, nframes = 250, res=100)
